import { LowPass1pBodePlot, LowPass2pBodePlot, NotchFilterBodePlot, HarmonicNotchBodePlot, AverageFilterBodePlot, DerivativeFilterBodePlot } from '@site/src/components/DspFilterBodePlot';
...
一阶低通滤波器 `alpha = (2π·fc/fs) / (1 + 2π·fc/fs)`，频率响应如下：
...
二阶 Butterworth 滤波器的幅度在截止频率处精确 −3 dB，滚降速率 −40 dB/decade，相位延迟最大 −180°：
...
extern "C" void GimbalNotchTask(const void *pv_arg) {
  ...
  for (;;) {
    ...
    float feedback = pitch_motor.pos_rad();
...
滑动平均滤波器响应为 sinc 函数，第一零点在 fs/N 处，增大 N 可获得更强的低频平滑效果：

<AverageFilterBodePlot />
...
extern "C" void VelocityEstimationTask(const void *pv_arg) {
  ...
  for (;;) {
    // 获取编码器角度
    float angle = motor.pos_rad();

    // 计算角速度
    float angular_velocity = vel_estimator.apply(angle, dt);

    // 对比电机内部的速度测量
    float motor_velocity = motor.rpm();
...
蓝色曲线为实际增益，橙色虚线为理想微分器（增益 ∝ f，斜率 +20 dB/decade）。两条曲线偏离超过 3 dB 的频率即为有效微分带宽上限：

<DerivativeFilterBodePlot />
...
extern "C" void GimbalSlewTask(const void *pv_arg) {
  ...
  for (;;) {
    float target = GetTargetAngle();
    pid.Update(target, yaw_motor.pos_rad());
