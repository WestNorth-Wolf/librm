...
本例程演示如何使用 librm 中的 `MotorPowerModel` 类实现电机功率估算和底盘功率控制。

`MotorPowerModel` 基于西交利物浦大学 RM2023 电机功率模型开源实现，通过拟合模型估算电机功率，可用于直接底盘功率限制或辅助降低超级电容的工作压力。
...
void CalculateMotorPower() {
  ...
  // 构建电机状态
  rm::modules::MotorPowerModel::MotorState state;
...
extern "C" void ChassisPowerControlTask(const void *pv_arg) {
  ...
  for (;;) {
    ...
    std::array<rm::modules::MotorPowerModel::MotorState, 4> motor_states;

    for (int i = 0; i < 4; i++) {
      velocity_pids[i].Update(target_speeds[i], motors[i].rpm());
      initial_currents[i] = velocity_pids[i].out();

      // 构建电机状态
      motor_states[i].speed_rpm = motors[i].rpm();
...
extern "C" void DynamicPowerControlTask(const void *pv_arg) {
  ...
  for (;;) {
    // 从裁判系统获取底盘功率限制
    float chassis_power_limit = referee.data().robot_status.chassis_power_limit;

    // 获取当前缓冲能量
    float buffer_energy = referee.data().power_heat_data.buffer_energy;
