import DiffFilterBodePlot from '@site/src/components/DiffFilterBodePlot';
...
for (;;) {
  ...
  // 外环输出作为内环目标
  position_pid.Update(target_pos, motor.encoder(), dt);
  float target_vel = position_pid.out();

  // 内环控制
  velocity_pid.Update(target_vel, motor.rpm(), dt);
...
该滤波器为一阶 IIR 低通滤波器，差分方程为 `y[n] = α·x[n] + (1-α)·y[n-1]`。
下方 Bode 图展示了不同 α 和采样率下的幅频/相频响应，可实时调节参数观察，指导实际使用：
...
for (;;) {
  ...
  float current_angle = motor.pos_rad();
