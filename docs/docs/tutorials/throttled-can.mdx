...
> 在 **`kFifo` 策略**下，传入的 `priority` 值会被忽略；`kPriorityFifo`（默认）和 `kEdf` 模式下优先级参数均生效。
...
`Write(id, data, size)` 的三参数重载使用默认 deadline，默认为入队时刻 **+ 50 ms**。可在运行时随时修改：

```cpp
using namespace std::chrono_literals;

// 方式一：传入任意 duration 类型
motor_can.SetDefaultDeadlineOffset(std::chrono::milliseconds(100));
motor_can.SetDefaultDeadlineOffset(20ms);  // 使用字面量

// 方式二：直接传入毫秒数（整数）
motor_can.SetDefaultDeadlineOffset(100);  // 100 ms

// 读取当前值
auto offset = motor_can.default_deadline_offset();
```

:::tip
调试 deadline 时建议参考 `stats_snapshot` 中的 `drop_expired_fps` 指标。
:::
