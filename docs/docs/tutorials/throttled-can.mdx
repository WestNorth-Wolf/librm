...
rm::hal::ThrottledCan<128> motor_can{hcan1, 3000};

// 严格 FIFO，完全按入队顺序发出（忽略 priority 参数）
rm::hal::ThrottledCan<128, rm::modules::SchedulingPolicy::kFifo> motor_can{hcan1, 3000};

// EDF：优先级大的先发，同优先级时 deadline 最早的先发
rm::hal::ThrottledCan<128, rm::modules::SchedulingPolicy::kEdf> motor_can{hcan1, 3000};
```
